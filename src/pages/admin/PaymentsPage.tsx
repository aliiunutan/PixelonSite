  ...
  const handleOpenModal = (payment: any = null) => {
    if (payment) {
      ...
      setFormData({
        ...
        odeme_tarihi: payment.odeme_tarihi || new Date().toISOString().split('T')[0],
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold text-slate-800">Ödeme & Fatura</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card p-6 bg-emerald-50 border-emerald-100">
          <p className="text-xs font-bold text-emerald-600 uppercase mb-1">Toplam Tahsilat</p>
          <h3 className="text-2xl font-bold text-emerald-900">₺{totalReceived.toLocaleString('tr-TR')}</h3>
        </div>
        <div className="card p-6 bg-amber-50 border-amber-100">
          <p className="text-xs font-bold text-amber-600 uppercase mb-1">Bekleyen Ödemeler</p>
          <h3 className="text-2xl font-bold text-amber-900">₺{totalPending.toLocaleString('tr-TR')}</h3>
        </div>
      </div>
                ...
                <tr key={payment.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-sm">{payment.fatura_no || `INV-${payment.id.substring(0, 6)}`}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <Building2 size={16} className="text-slate-400" />
                      <div className="font-semibold text-slate-900">{payment.firma_adi}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-900">₺{payment.toplam_tutar.toLocaleString('tr-TR')}</div>
                    {payment.kapora > 0 && <div className="text-[10px] text-emerald-600 font-bold">Kapora: ₺{payment.kapora}</div>}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase flex items-center gap-1 w-fit ${
                      payment.odeme_durumu === 'tamamlandi' ? 'bg-emerald-100 text-emerald-700' :
                      payment.odeme_durumu === 'kismi_odendi' ? 'bg-amber-100 text-amber-700' :
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => handleOpenModal(payment)} className="p-2 text-slate-400 hover:text-primary rounded-lg transition-colors">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(payment.id)} className="p-2 text-slate-400 hover:text-rose-600 rounded-lg transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">İlgili Çekim</label>
            <select 
              required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Toplam Tutar (TL)</label>
              <input 
                type="number" 
                step="0.01"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Kapora (TL)</label>
              <input 
                type="number" 
                step="0.01"
