    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredTasks.map((task) => (
          <div key={task.id} className={`card p-6 flex items-start gap-4 hover:border-primary/30 transition-colors ${task.durum === 'tamamlandi' ? 'opacity-60' : ''}`}>
            <button 
              onClick={() => toggleComplete(task)}
              className={`mt-1 transition-colors ${task.durum === 'tamamlandi' ? 'text-emerald-500' : 'text-slate-300 hover:text-primary'}`}
            >
              {task.durum === 'tamamlandi' ? <CheckCircle size={24} /> : <Circle size={24} />}
            </button>
            <div className="flex-grow">
              <div className="flex justify-between items-start mb-1">
                <h3 className={`font-bold text-slate-900 ${task.durum === 'tamamlandi' ? 'line-through' : ''}`}>{task.gorev_adi}</h3>
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                  task.durum === 'tamamlandi' ? 'bg-emerald-100 text-emerald-700' :
                  task.durum === 'beklemede' ? 'bg-amber-100 text-amber-700' :
                  'bg-slate-100 text-slate-700'
                }`}>
                  {task.durum}
                </span>
              </div>
              <p className="text-sm text-slate-500 mb-3">{task.aciklama}</p>
            ...
            <div className="flex gap-1">
              <button onClick={() => handleOpenModal(task)} className="p-2 text-slate-400 hover:text-primary rounded-lg transition-colors">
                <Edit2 size={16} />
              </button>
              <button onClick={() => handleDelete(task.id)} className="p-2 text-slate-400 hover:text-rose-600 rounded-lg transition-colors">
                <Trash2 size={16} />
              </button>
            </div>
          </div>
